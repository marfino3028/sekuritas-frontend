      ...
      <div class="relative px-4 py-4 border-t border-white/10">
        <div class="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-white/10 transition-colors cursor-pointer">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs truncate text-indigo-200">{{ authStore.user?.phone }}</p>
          </div>
        </div>
      </div>
    </aside>
      ...
      <aside class="absolute left-0 top-0 h-full w-72 flex flex-col bg-brand-gradient overflow-hidden">
        ...
        <div class="relative flex items-center justify-between px-5 py-5 border-b border-white/10">
          ...
          <button @click.stop="mobileMenuOpen = false"
                  class="p-1.5 rounded-xl text-indigo-200 hover:text-white hover:bg-white/10 transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <nav class="relative flex-1 px-3 py-5 space-y-1.5">
          <NuxtLink
            ...
            :class="isActive(item.to) ? 'text-white bg-white/15 ring-1 ring-white/20 shadow-soft' : 'text-indigo-200 hover:text-white hover:bg-white/10'"
          >
            <component :is="item.icon" class="w-5 h-5 flex-shrink-0" />
            {{ item.label }}
          </NuxtLink>
        ...
        <div class="relative px-4 py-4 border-t border-white/10">
          <button
            @click="handleLogout"
            class="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium text-indigo-200 hover:text-white hover:bg-white/10 transition-all"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
